// Self-Contained, 100% Reliable UTF-8 QR Code Engine for DOTRA Gate System
// محرك توليد رموز QR الحديث والمدمج - يدعم الحروف العربية والإنجليزية دون أي مكتبات خارجية

(function (global) {
    'use strict';

    // TypeTable, Error Correction & Reed-Solomon polynomial math
    var QRMode = { NUMBER: 1, ALPHA_NUM: 2, BYTE: 4, KANJI: 8 };
    ...
    QRCodeModel.prototype = {
        addData: function (data) {
            var qrByte = new QR8bitByte(data);
            this.dataList.push(qrByte);
            this.dataCache = null;
        },
    ...
    /**
     * Primary Global QR Engine API
     */
    var QREngine = {
        /**
         * Render QR Code into DOM container element
         */
        render: function (container, text, options) {
            options = options || {};
            var size = options.size || 150;
            var colorDark = options.colorDark || '#002b66';
            var colorLight = options.colorLight || '#ffffff';
            var ecLevel = options.errorCorrectLevel !== undefined ? options.errorCorrectLevel : QRErrorCorrectLevel.M;
                ...
                for (var r = 0; r < moduleCount; r++) {
                    for (var c = 0; c < moduleCount; c++) {
                        if (qr.isDark(r, c)) {
                            ...
                            var w = Math.ceil((c + 1) * cellSize) - x;
                            var h = Math.ceil((r + 1) * cellSize) - y;
        ...
        /**
         * Render directly to an existing Canvas Context (for badges & pass sharing)
         */
        drawToCanvas: function (ctx, text, x, y, size, colorDark, colorLight) {
            colorDark = colorDark || '#002b66';
            colorLight = colorLight || '#ffffff';
            try {
                var typeNum = findBestTypeNumber(text, QRErrorCorrectLevel.M);
                ...
                for (var r = 0; r < moduleCount; r++) {
                    for (var c = 0; c < moduleCount; c++) {
                        if (qr.isDark(r, c)) {
                            var cellX = Math.round(x + c * cellSize);
                            var cellY = Math.round(y + r * cellSize);
                            var w = Math.ceil(x + (c + 1) * cellSize) - cellX;
                            var h = Math.ceil(y + (r + 1) * cellSize) - cellY;
    ...
    global.QRCode = function (el, opt) {
        if (typeof opt === 'string') opt = { text: opt };
        return QREngine.render(el, opt.text, opt);
    };
